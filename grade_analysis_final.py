...
def extract_credits_from_course_name(course_name):
    """
    从课程名称中提取学分数
    例如：花卉栽培与环境 【1.0】 -> 1.0
    """
...
def process_combined_data(file_paths):
    ...
    for file_path in file_paths:
        ...
        for col in df.columns:
            if '【' in str(col) and '】' in str(col):
                # 提取学分确保是有效课程
                credits = extract_credits_from_course_name(col)
                if credits > 0:
        ...
        for index, row in df.iterrows():
            ...
            for course_col in course_columns:
                # 提取课程学分
                credits = extract_credits_from_course_name(course_col)
                if credits == 0:
                    continue
                    
                # 获取成绩
                grade = row[course_col]
                score = convert_grade_to_score(grade)
    ...
    for student_id, student_data in all_students_data.items():
        ...
        for course_key, course_data in student_data['courses'].items():
            ...
            for major_course in main_courses:
...
def main():
    # 文件路径
    data_dir = "data"
    file1 = os.path.join(data_dir, "数据科学231-1.xlsx")
    file2 = os.path.join(data_dir, "数据科学231-2.xlsx")
    main_course_file = os.path.join(data_dir, "主要课程列表-数据科学231.xlsx")
    global main_courses

    # 读取主要课程列表
    main_courses = pd.read_excel(main_course_file)['主要课程'].tolist()
    # print(professional_courses)

    print("=== 学分成绩表分析系统 ===")
    
    # 合并处理两个学期的数据
    file_paths = [file1, file2]
    combined_results = process_combined_data(file_paths)
    ...
    # 按照学分加权平均分降序排列
    combined_results = combined_results.sort_values(by='学分加权平均分', ascending=False)

    print("\n=== 保存结果 ===")
    with pd.ExcelWriter('成绩分析结果_详细版.xlsx', engine='openpyxl') as writer:
        combined_results.to_excel(writer, sheet_name='综合成绩', index=False)
    
    print(f"\n分析完成！结果已保存到 '成绩分析结果_详细版.xlsx'")
    print(f"共处理 {len(combined_results)} 名学生的成绩数据")
